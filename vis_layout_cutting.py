...
if __name__ == '__main__':
    ...
    # 추가
    d2_points = cor_id.astype(np.int32)
    d2_to_d3_points = xyzrgb[d2_points[:,1], d2_points[:,0], :3]
    d2_to_d3_points_color = xyzrgb[d2_points[:, 1], d2_points[:, 0], 3:]

    p1 = d2_to_d3_points[0]
    p2 = d2_to_d3_points[1]
    p3 = d2_to_d3_points[2]
    ...
    # 평면 위의 모든 점 추출
    # 평면의 방정식은 Ax + By + Cz + D = 0 형태
    # 따라서 Ax + By + Cz + D 값이 0에 가까운 점을 평면 위의 점으로 간주할 수 있습니다.
    threshold = 0.15  # 0에 가까운 값으로 설정 (원하는 정확도에 따라 조절 가능)
    xyzrgb_flatten = np.reshape(xyzrgb, (-1, 6))
    xyz_flatten = xyzrgb_flatten[:, :3]
    plane_points = xyz_flatten[np.abs(np.dot(xyz_flatten, plane_equation[:3]) + plane_equation[3]) < threshold]
...
fig = plt.figure()
# 3d 그래프를 그리기 위해 Axes3D를 임포트합니다.
from mpl_toolkits.mplot3d import Axes3D
ax = fig.add_subplot(111, projection='3d')

# plane_points를 x, y, z로 나눕니다.
x = plane_points[:, 0]
y = plane_points[:, 1]
z = plane_points[:, 2]
