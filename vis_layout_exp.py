...
if __name__ == '__main__':
    ...
    args = parser.parse_args(["--img", "./assets/pano_asmasuxybohhcj.png",
                              "--layout", "./assets/HoHoNet_layout.txt"])
    ...
    for i in range(0, len(cor_id)-2, 2):
        point4 = cor_id[i:i+4, :]
        xs = point4[:, 0].astype(int)
        ys = point4[:, 1].astype(int)
        point4_xyz = xyzrgb[ys, xs, :3]

        idx_x = np.where(point4_xyz[0][0]<xyzrgb[0]<point4_xyz[0][1] &  point4_xyz[1][0]<xyzrgb[0]<point4_xyz[1][1])
